...
export function CareCloudSyncPanel({ onSyncApplied }) {
  ...
  const applyResult = useCallback(
    (label, result) => {
      ...
      } else if (merged > 0 || pulled > 0 || Number(result?.storesMerged ?? 0) > 0) {
        const storePart =
          Number(result?.storesMerged ?? 0) > 0 ? `・予定${Number(result.storesMerged)}件` : '';
        setLastMsg(`${label}: ${merged}件反映（クラウド ${pulled}件${storePart}）`);
        onSyncApplied?.();
      } else if (upserted > 0) {
  ...
  useEffect(() => {
    ...
    const onSync = (ev) => {
      ...
      if (Number(d?.merged ?? 0) > 0 || Number(d?.storesMerged ?? 0) > 0) {
        const storePart =
          Number(d?.storesMerged ?? 0) > 0 ? `（予定カレンダー ${Number(d.storesMerged)}件）` : '';
        setLastMsg(`自動同期: ${Number(d?.merged ?? 0)}件を反映しました${storePart}`);
        onSyncApplied?.();
